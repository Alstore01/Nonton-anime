...
async function animeterbaru(page = 1) {
  const res = await axios.get(`https://cors.caliph.my.id/https://v18.kuramanime.ing/anime/page/${page}/`, { headers });
...
async function search(query) {
  const res = await axios.get(`https://cors.caliph.my.id/https://v18.kuramanime.ing/?s=${encodeURIComponent(query)}`, { headers });
  const $ = cheerio.load(res.data);
  const data = [];
  $('.animpost').each((_, e) => {
    data.push({
...
async function detail(link) {
  ...
  return {
    title: $('title').text().replace(' - Kuramanime', '').trim(),
...
async function download(link) {
  ...
  for (const li of $('div#server > ul > li').toArray()) {
    ...
    try {
        const r = await axios.post('https://cors.caliph.my.id/https://v18.kuramanime.ing/wp-admin/admin-ajax.php', body, {
        headers: {
            ...headers,
